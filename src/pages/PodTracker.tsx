...
function PodSubmissionCard({
  ...
  return (
    <div
      className="bg-s1 border border-border rounded-xl overflow-hidden hover:border-border2 transition-all"
    ...
    >
      ...
      {submission.media_type === 'audio' && submission.file_data && (
        <div className="border-b border-border bg-s2 p-3">
          <audio
            controls
            src={`/api/pod-media/${submission.id}`}
            className="w-full"
      ...
      {submission.media_type === 'document' && submission.file_data && (
        <div className="border-b border-border bg-s2 p-3 flex items-center justify-center">
          <a
            href={`/api/pod-media/${submission.id}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 text-blue text-xs font-medium hover:underline"
          >
      ...
      <div className="p-3.5">
        ...
        {/* Caption */}
        {submission.caption && (
          <p className="text-xs text-muted/80 leading-relaxed mb-2 whitespace-pre-wrap">{submission.caption}</p>
        )}
